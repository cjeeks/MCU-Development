...
//
//*****************************************************************************
extern void UARTStdioInit(unsigned long ulPortNum, unsigned int baud);
extern void UARTStdioInitExpClk(unsigned long ulPort, unsigned long ulBaud);
extern int UARTgets(char *pcBuf, unsigned long ulLen);		//接收字符串
extern unsigned char UARTgetc(void);						 //接收字符
extern void UARTprintf(const char *pcString, ...);			  //打印信息
extern int UARTwrite(const char *pcBuf, unsigned long ulLen); //输入字符串
#ifdef UART_BUFFERED
